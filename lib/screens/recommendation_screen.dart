...
import '../services/tts_service.dart'; // Make sure this file exists from the previous step
...
class _RecommendationScreenState extends State<RecommendationScreen> {
  ...
  void dispose() {
    ...
    _ttsService.dispose(); // ── Stop audio when leaving the screen ──
    super.dispose();
  }
  ...
  void _toggleSpeech() {
    if (_isSpeaking) {
      ...
      _ttsService.speakRecommendations(widget.response);
    }
  }

  // ── Download full trip report as PDF ──
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ── ADDED: Floating Action Button for Text-to-Speech ──
      // ── UPDATED: Smaller Voice Button without text ──
      floatingActionButton: FloatingActionButton(
        backgroundColor: AppTheme.saffron,
        onPressed: _toggleSpeech,
        // Optional: uncomment the line below if you want it even tinier!
        // mini: true,
        child: Icon(
      ...
      body: CustomScrollView(
        slivers: [
          ...
          ), // Added extra padding so FAB doesn't cover tips
        ],
      ),
    );
  }
...
class _DownloadReportButton extends StatelessWidget {
  ...
  const _DownloadReportButton({required this.isLoading, required this.onTap});
